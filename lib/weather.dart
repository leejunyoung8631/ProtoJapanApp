...
class WholeWeather {
  ...
  void getHourlyList(var whole) {
    hourlyList = List<Weather>.generate(8, (index) {
...
class Weather {
  ...
  Weather(var whole, {int id = 0, isDaily = true}) {
    ...
    if (isDaily == true) {
      av_temp = whole["daily"][id]["temp"]["day"].toString();
      min_temp = whole["daily"][id]["temp"]["min"].toString();
      max_temp = whole["daily"][id]["temp"]["max"].toString();
      ...
      av_temp = whole["hourly"][id]["temp"];
      humidity = whole["hourly"][id]["humidity"];
      weather = whole["hourly"][id]["weather"][0]["main"];
      weather_path = "assets/images/$weather.png";
